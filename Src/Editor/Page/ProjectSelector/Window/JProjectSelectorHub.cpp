...
namespace JinEngine
{
	namespace Editor
	{
		...
		{
			JEditor::DoActivate();
			serachIconTexture = Core::GetUserPtr(JResourceManager::Instance().GetEditorTexture(J_EDITOR_TEXTURE::SEARCH_FOLDER_ICON));
			CallOnResourceReference(serachIconTexture.Get());

			backgroundTexture = Core::GetUserPtr(JResourceManager::Instance().GetEditorTexture(J_EDITOR_TEXTURE::PROJECT_SELECTOR_BACKGROUND));
