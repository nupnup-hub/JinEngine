...
namespace JinEngine
{
	namespace Core
	{
		JGraphicException::JGraphicException(int line, const std::string file, HRESULT hr)
			:JException(line, JCommonUtility::StringToWstring(file)), hr(hr), gfxHr(JGraphic::Instance().DeviceInterface()->GetDevice()->GetDeviceRemovedReason())
		{
			std::ostringstream oss;
			oss << std::hex << gfxHr << std::endl;
			hexerrorcode = oss.str();
			//MessageBox(0, JWindow::Instance().HrException::TranslateErrorCode(gfxHr).c_str(), (L"\n[Error Code] 0x" + JCommonUtility::StringToWstring(hexerrorcode)).c_str(), 0);
		...
		{
			std::wstring oss;
			oss = GetType() + L"\n[Error Code] 0x" + JCommonUtility::StringToWstring(hexerrorcode)
