...
namespace JinEngine
{
	ObjectDetail::ObjectDetail(std::unique_ptr<EditorAttribute> attribute, const size_t ownerPageGuid)
	...
	void ObjectDetail::Initialize(EditorUtility* editorUtility)noexcept
	{
	}
	void ObjectDetail::UpdateWindow(EditorUtility* editorUtility)
	{
	...
	bool ObjectDetail::Activate(EditorUtility* editorUtility)
	{
		if (Editor::Activate(editorUtility))
		{
			std::vector<EDITOR_EVENT> enumVec
			{
				EDITOR_EVENT::MOUSE_CLICK,EDITOR_EVENT::SELECT_RESOURCE,EDITOR_EVENT::SELECT_GAMEOBJECT, EDITOR_EVENT::DESELECT_GAMEOBJECT
			};
	...
	bool ObjectDetail::DeActivate(EditorUtility* editorUtility)
	{
	...
	bool ObjectDetail::OnFocus(EditorUtility* editorUtility)
	{
	...
	bool ObjectDetail::OffFocus(EditorUtility* editorUtility)
	{
	...
	void ObjectDetail::SelectedObjectDetailOnScreen(EditorUtility* editorUtility)
	{
	...
	void ObjectDetail::TransformDetailOnScreen(EditorUtility* editorUtility, JGameObject* gameObj)
	{
		if (gameObj != nullptr)
		{
	...
	void ObjectDetail::AnimatorDetailOnScreen(EditorUtility* editorUtility, JAnimator* animator)
	{
	...
	void ObjectDetail::CameraDetailOnScreen(EditorUtility* editorUtility, JCamera* camera)
	{
	...
	void ObjectDetail::LightDetailOnScreen(EditorUtility* editorUtility, JLight* light)
	{
	...
	void ObjectDetail::ShowComponentSelector(EditorUtility* editorUtility, JGameObject* gameObj)
	{
		if (gameObj != nullptr)
		{
			...
			{
				uint componentCount;
				std::vector<JComponentData*>::const_iterator st = JComponentDatabase::GetComponnetDataVec(componentCount);
				int selectIndex = -1;
				...
				{ 
					if (ImGui::Selectable((*(st + i))->componentName.c_str()))
						selectIndex = i;
				}
				ImGui::EndListBox();
				...
				if (selectIndex != -1)
				{
					JComponentData* select = *(st + selectIndex); 
					JComponentFactoryUtility::CreateComponent(select->componentName, *gameObj);
					selectorEvVec.push_back(SELECTOR_STATE::CLOSE);
				}
			}
		}
	}
	void ObjectDetail::MeshDetailOnScreen(EditorUtility* editorUtility, JMeshGeometry* mesh, const J_OBJECT_TYPE objType)
	{
	...
	void ObjectDetail::MaterialDetailOnScreen(EditorUtility* editorUtility, JMaterial* material, const J_OBJECT_TYPE objType)
	{
		...
		if (material != nullptr)
		{
			...
			JOBJECT_FLAG objFlag = material->GetFlag();
			bool editable = ((int)objFlag & OBJECT_FLAG_UNEDITABLE) == 0;

			Vector4<float> oldAlbedoColor = material->GetAlbedoColor();
			...
			Vector4 newAlbedoColor(materialDetail.albedoColorBuff[0],
			...
			if (editable)
			{
				...
				{
				case JinEngine::ObjectDetail::SELECTOR_STATE::OPEN_TEXTURE_ALBEDO:
				{
					JResourceObject* res = nullptr;
				...
				break;
				case JinEngine::ObjectDetail::SELECTOR_STATE::OPEN_TEXTURE_NORMAL:
				{
					JResourceObject* res = nullptr;
					if (ResourceSelectorOnScreen(editorString->GetString(materialDetail.textureWindowId).c_str(), editorUtility, &res))
					{
				...
				break;
				case JinEngine::ObjectDetail::SELECTOR_STATE::OPEN_TEXTURE_HEIGHT:
				{
					JResourceObject* res = nullptr;
					if (ResourceSelectorOnScreen(editorString->GetString(materialDetail.textureWindowId).c_str(), editorUtility, &res))
					{
				...
				break;
				case JinEngine::ObjectDetail::SELECTOR_STATE::OPEN_TEXTURE_ROUGHNESS:
				{
					JResourceObject* res = nullptr;
					if (ResourceSelectorOnScreen(editorString->GetString(materialDetail.textureWindowId).c_str(), editorUtility, &res))
					{
				...
				break;
				case JinEngine::ObjectDetail::SELECTOR_STATE::OPEN_TEXTURE_AMBIENT:
				{
					JResourceObject* res = nullptr;
					if (ResourceSelectorOnScreen(editorString->GetString(materialDetail.textureWindowId).c_str(), editorUtility, &res))
					{
	...
	void ObjectDetail::ModelDataOnScreen(EditorUtility* editorUtility, JModel* model, const J_OBJECT_TYPE objType)
	{
	...
	void ObjectDetail::AnimationClipDataOnScreen(EditorUtility* editorUtility, JAnimationClip* animationClip, const J_OBJECT_TYPE objType)
	{
	...
	void ObjectDetail::TextureDataOnScreen(EditorUtility* editorUtility, JTexture* texture, const J_OBJECT_TYPE objType)
	{
	...
	void ObjectDetail::SkeletonAssetDataOnScreen(EditorUtility* editorUtility, JSkeletonAsset* skeletonAsset, const J_OBJECT_TYPE objType)
	{
	...
	bool ObjectDetail::ResourceSelectorOnScreen(const std::string& windowName, _In_ EditorUtility* editorUtility, _Out_ JResourceObject** select)
	{
	...
	void ObjectDetail::SearchSelectorEvVec()
	{
	...
	bool ObjectDetail::OpenSelectorWindow(const SELECTOR_STATE selectorState)
	{
		J_RESOURCE_TYPE matchResourceType = J_RESOURCE_TYPE::NONE;
		switch (selectorState)
		{
		case JinEngine::ObjectDetail::SELECTOR_STATE::OPEN_MESH:
			matchResourceType = J_RESOURCE_TYPE::MESH;
			break;
		case JinEngine::ObjectDetail::SELECTOR_STATE::OPEN_MATERIAL:
		...
		case JinEngine::ObjectDetail::SELECTOR_STATE::OPEN_TEXTURE_AMBIENT:
			matchResourceType = J_RESOURCE_TYPE::TEXTURE;
			break;
		case JinEngine::ObjectDetail::SELECTOR_STATE::OPEN_ANIMATION_CONTROLLER:
			matchResourceType = J_RESOURCE_TYPE::ANIMATION_CONTROLLER;
			break;
		case JinEngine::ObjectDetail::SELECTOR_STATE::OPEN_SKELETON:
		...
		case J_RESOURCE_TYPE::ANIMATION_CLIP:
		{
			...
			for (uint i = 0; i < count; ++i)
			{
				if (((*(st + i))->GetFlag() & OBJECT_FLAG_HIDDEN) == 0)
					JResourceManager::Instance().CreatePreviewScene(previewGroup, *(st + i), PREVIEW_DIMENSION::TWO_DIMENTIONAL_RESOURCE);
			}
			return true;
		}
	...
	void ObjectDetail::Spacing()
	{
		ImGui::Spacing(); ImGui::Spacing(); ImGui::Spacing(); ImGui::Spacing(); ImGui::Spacing();
	}
	void ObjectDetail::OnEvent(const size_t& senderGuid, const EDITOR_EVENT& eventType, EditorEventStruct* eventStruct)
	{
		...
		if (eventType == EDITOR_EVENT::MOUSE_CLICK)
		{
			...
			selectorEvVec.push_back(SELECTOR_STATE::CLOSE);
		else if (eventType == EDITOR_EVENT::DESELECT_GAMEOBJECT)
			selectorEvVec.push_back(SELECTOR_STATE::CLOSE);
	}
}