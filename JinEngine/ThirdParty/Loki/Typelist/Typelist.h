...
#define DECLARE_TYPE_LIST_FUNC(thisClass)                                               \
        public:                                                                         \
		    static size_t TypeGuid()noexcept                                            \
		    {                                                                           \
			    static size_t guid = std::hash<std::string>{}(typeid(thisClass).name()); \
			    return guid;                                                            \
		    }                                                                           \
            static bool IsA(const size_t typeGuid)noexcept { return TypeGuid() == typeGuid; }\
		    virtual bool IsChildOf(const size_t typeGuid)const noexcept                 \
		    {                                                                           \
                return IsChildOf(std::make_index_sequence<Length<ThisTypeList>::value>(), typeGuid);  \
		    }                                                                           \
        private:                                                                        \
            template<size_t ...Is>                                                      \
            bool IsChildOf(std::index_sequence<Is...>, const size_t typeGuid)const noexcept         \
            {                                                                           \
                return (TypeAt<ThisTypeList, Is>::Result::IsA(typeGuid) && ...);        \
            }                                                                           \



#define DECLARE_BASE_CLASS_TYPE_LIST(thisClass) protected: using ThisTypeList = typename MakeTypelist<thisClass>::Result;\
        DECLARE_TYPE_LIST_FUNC(thisClass)   \


#define DECLARE_CHILD_CLASS_TYPE_LIST(thisClass, parentClass) protected: using ThisTypeList = typename Append<parentClass::ThisTypeList, thisClass>::Result;\
        DECLARE_TYPE_LIST_FUNC(thisClass)   \
