...
namespace JinEngine
{
    template<typename T> struct JVector2;
}
struct ImVec2
{
    float                                   x, y;
    ...
    template<typename T>
    ImVec2(const JinEngine::JVector2<T>& jVec2);
    template<typename T>
    ImVec2& operator=(const JinEngine::JVector2<T>& jVec2);

    float  operator[] (size_t idx) const { IM_ASSERT(idx <= 1); return (&x)[idx]; }    // We very rarely use this [] operator, the assert overhead is fine.
    float& operator[] (size_t idx) { IM_ASSERT(idx <= 1); return (&x)[idx]; }    // We very rarely use this [] operator, the assert overhead is fine.

    bool operator!=(const ImVec2& newSize)
    {
        if (x != newSize.x)
            return true;
        else if (y != newSize.y)
    ...
    const ImVec2& operator+(const ImVec2& target)
    {
        return ImVec2(x + target.x, y + target.y);
    }
    ImVec2 operator-(const ImVec2& target)
    {
        return ImVec2(x - target.x, y - target.y);
    }
    ImVec2 operator*(const float scale)
    {
...
namespace JinEngine
{
    template<typename T> struct JVector3;
    template<typename T> struct JVector4;
}
struct ImVec4
{
    float                                           x, y, z, w;
    ...
    template<typename T>
    ImVec4(const JinEngine::JVector4<T>& jVec4);
    template<typename T>
    ImVec4(const JinEngine::JVector3<T>& jVec3, const T _w);
    template<typename T>
    ImVec4& operator=(const JinEngine::JVector4<T>& jVec4); 
...
namespace ImGui
{
    ...
    IMGUI_API void          SetNextWindowContentSize(const ImVec2& size);                               // set next window content size (~ scrollable client area, which enforce the length of scrollbars). Not including window decorations (title bar, menu bar, etc.) nor WindowPadding. set an axis to 0.0f to leave it automatic. call before Begin()
    ...
    IMGUI_API bool          SliderInt(const char* label, unsigned int* v, unsigned int v_min, unsigned int v_max, const char* format = "%d", ImGuiSliderFlags flags = 0);
    ...
    IMGUI_API bool          VSliderInt(const char* label, const ImVec2& size, unsigned int* v, unsigned int v_min, unsigned int v_max, const char* format = "%d", ImGuiSliderFlags flags = 0);
    ...
    IMGUI_API bool          InputInt(const char* label, unsigned int* v, int step = 1, int step_fast = 100, ImGuiInputTextFlags flags = 0);
...
// The clipper calculates the length of visible items and advance the cursor to compensate for the non-visible items we have skipped.
...
// - Clipper calculate the actual length of elements to display based on the current clipping rectangle, position the cursor before the first visible element.
...
struct ImGuiListClipper
{
    ...
    // Call ForceDisplayRangeByIndices() before first call to Step() if you need a length of items to be displayed regardless of visibility.
    IMGUI_API void  ForceDisplayRangeByIndices(int item_min, int item_max); // item_max is exclusive e.g. use (42, 42+1) to make item 42 always visible BUT due to alignment/padding of certain items it is likely that an extra item may be included on either end of the display length.
...
#pragma region Custom
    ...
    IMGUI_API void AddRectFilledMultiColor(const ImVec2& p_min, const ImVec2& p_max, const ImVec4& col_upr_left, const ImVec4& col_upr_right, const ImVec4& col_bot_right, const ImVec4& col_bot_left, float rounding, ImDrawFlags flags);
    IMGUI_API void AddInvertedRectFilled(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding, ImDrawFlags rounding_corners);
    IMGUI_API void PathInvertedRect(const ImVec2& a, const ImVec2& b, ImU32 col, float rounding, ImDrawFlags rounding_corners);
#pragma endregion
...
struct ImFontConfig
{
    ...
    const ImWchar*  GlyphRanges;            // NULL     // Pointer to a user-provided list of Unicode length (2 value per length, values are inclusive, zero-terminated list). THE ARRAY DATA NEEDS TO PERSIST AS LONG AS THE FONT IS ALIVE.
...
struct ImFontAtlas
{
    ...
    // Helpers to retrieve list of common Unicode ranges (2 value per length, values are inclusive, zero-terminated list)
...
{
    //Custom
    IMGUI_API bool          BeginComboEx(const char* label, const char* preview_value, ImGuiComboFlags flags = 0, ImGuiDir_ initArrowDir = ImGuiDir_Down, ImGuiDir_ activateArrowDir = ImGuiDir_Down);
}
