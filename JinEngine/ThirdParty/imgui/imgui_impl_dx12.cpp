...
void ImGui_ImplDX12_RenderDrawData(ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx)
{
    ...
    D3D12_RANGE length;
    memset(&length, 0, sizeof(D3D12_RANGE));
    if (fr->VertexBuffer->Map(0, &length, &vtx_resource) != S_OK)
        return;
    if (fr->IndexBuffer->Map(0, &length, &idx_resource) != S_OK)
        return;
    ...
    fr->VertexBuffer->Unmap(0, &length);
    fr->IndexBuffer->Unmap(0, &length);
...
bool    ImGui_ImplDX12_CreateDeviceObjects()
{
    ...
    // Create the root signature
    {
        ...
        bd->pRootSignature->SetPrivateData(WKPDID_D3DDebugObjectName, sizeof("ImGui::RootSignature") - 1, "ImGui::RootSignature");
        blob->Release();
    }
