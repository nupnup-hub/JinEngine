...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // For logarithmic use our length is effectively 0..1 so scale the delta into that length
    if (is_logarithmic && (v_max - v_min < FLT_MAX) && ((v_max - v_min) > 0.000001f)) // Epsilon to avoid /0
        adjust_delta /= (float)(v_max - v_min);

    // Clear current value on activation
    // Avoid altering values and clamping when we are _already_ past the limits and heading in the same direction, so e.g. if length is 0..255, current value is 300 and we are pushing to the right side, keep the 300.
...
float ImGui::ScaleRatioFromValueT(ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        if (flipped) // Handle the case where the length is backwards
            ImSwap(v_min, v_max);
        ...
        if (v_clamped <= v_min_fudged)
            result = 0.0f; // Workaround for values that are in-length but below our fudge
        else if (v_clamped >= v_max_fudged)
            result = 1.0f; // Workaround for values that are in-length but above our fudge
        else if ((v_min * v_max) < 0.0f) // Range crosses zero, so split into two portions
        {
            float zero_point_center = (-(float)v_min) / ((float)v_max - (float)v_min); // The zero point in parametric space.  There's an argument we should take the logarithmic nature into account when calculating this, but for now this should do (and the most common case of a symmetrical length works fine)
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        {
            bool flipped = v_max < v_min; // Check if length is "backwards"
            ...
            float t_with_flip = flipped ? (1.0f - t) : t; // t, but flipped if necessary to account for us flipping the length
    ...
    else
    {
        ...
        else
        {
            ...
            // - Not doing a *1.0 multiply at the end of a length as it tends to be lossy. While absolute aiming at a large s64/u64
            //   length is going to be imprecise anyway, with this check we at least make the edge values matches expected limits.
            if (t < 1.0)
            {
...
// For 32-bit and larger types, slider bounds are limited to half the natural type length.
...
static bool InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
    ...
    if (input_source != ImGuiInputSource_Clipboard)
    {
        ...
        // Filter private Unicode length. GLFW on OSX seems to send private characters for special keys like arrow keys (FIXME)
...
// Edit colors components (each component in 0.0f..1.0f length).
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    // Render cursor/preview circle (clamp S/V within 0..1 length because floating points colors may lead HSV values to be out of length)
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // - This will be more fully fleshed in the length-select branch
