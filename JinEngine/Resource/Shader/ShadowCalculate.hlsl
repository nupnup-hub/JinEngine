...
float4 CalculateLight(Material mat, float3 posW, float3 normalW, float3 tangentW, float3 toEye)
{
	float3 directLight = float3(0, 0, 0);
	for (int dLitIndex = directionalLitSt; dLitIndex < directionalLitEd; ++dLitIndex)
	{ 
		DirectionalLight dLitCalData;
		dLitCalData.direction = directionalLight[dLitIndex].direction;
		dLitCalData.color = directionalLight[dLitIndex].color;
		...
		if (directionalLight[dLitIndex].shadowMapType == DIRECTONAL_LIGHT_HAS_NORMAL_SHADOW_MAP)
		{
			...
			PCSSData pcssData;
			pcssData.frustumNear = directionalLight[dLitIndex].frustumNear;
			pcssData.frustumFar = directionalLight[dLitIndex].frustumFar;
			pcssData.penumbraScale = directionalLight[dLitIndex].penumbraScale;
			pcssData.penumbraBlockerScale = directionalLight[dLitIndex].penumbraBlockerScale;
			pcssData.tanAngle = directionalLight[dLitIndex].tanAngle;
			pcssData.bias = directionalLight[dLitIndex].bias;
			pcssData.sampleSize = directionalLight[dLitIndex].shadowMapInvSize; 
	
			shadowFactor = CalSSMUsePCSS(pcssData, shadowMaps[directionalLight[dLitIndex].shadowMapIndex], shadowPosH, posW);
#elif USE_DIRECTIONAL_LIGHT_PCF
			PCFData pcfData;
			pcfData.bias = directionalLight[dLitIndex].bias;
			pcfData.sampleSize = directionalLight[dLitIndex].shadowMapInvSize;
	
			shadowFactor = CalSSMUsePCF(pcfData, shadowMaps[directionalLight[dLitIndex].shadowMapIndex], shadowPosH);
#else
			shadowFactor = GetSSM(shadowMaps[directionalLight[dLitIndex].shadowMapIndex], shadowPosH, directionalLight[dLitIndex].bias);
#endif 
		}
		else if (directionalLight[dLitIndex].shadowMapType == DIRECTONAL_LIGHT_HAS_CSM)
		{
			...
			int csmDataIndex = directionalLight[dLitIndex].csmDataIndex + csmLocalIndex;
			int shadowMapIndex = directionalLight[dLitIndex].shadowMapIndex + csmLocalIndex;
#ifdef USE_DIRECTIONAL_LIGHT_PCSS
			PCSSData pcssData;
			pcssData.frustumNear = directionalLight[dLitIndex].frustumNear;
			pcssData.frustumFar = directionalLight[dLitIndex].frustumFar;
...
#else
			shadowFactor = GetCsm(shadowArray[shadowMapIndex], posV, directionalLight[dLitIndex].bias, csmDataIndex);
#endif  
		} 
#endif 
		directLight += ComputeDirectionalLight(dLitCalData, mat, normalW, tangentW, toEye) * shadowFactor;
	}
	for (int pLitIndex = pointLitSt; pLitIndex < pointLitEd; ++pLitIndex)
	{ 
		PointLight pLitCalData;
		pLitCalData.midPosition = pointLight[pLitIndex].midPosition;
		pLitCalData.sidePosition[0] = pointLight[pLitIndex].sidePosition[0];
		pLitCalData.sidePosition[1] = pointLight[pLitIndex].sidePosition[1];
		pLitCalData.color = pointLight[pLitIndex].color;
		pLitCalData.power = pointLight[pLitIndex].power;
		pLitCalData.frustumFar = pointLight[pLitIndex].frustumFar;
		
		float shadowFactor = 1.0f;
#ifdef SHADOW
		float3 litVec =  posW - pointLight[pLitIndex].midPosition; 
		if (pointLight[pLitIndex].hasShadowMap)
		{		
			int shadowMapIndex = pointLight[pLitIndex].shadowMapIndex; 
			...
			smData.sideLitVec *= pointLight[pLitIndex].shadowMapInvSize;
			smData.upLitVec *= pointLight[pLitIndex].shadowMapInvSize;
			
			float4 shadowCoord = mul(float4(posW, 1.0f), pointLight[pLitIndex].shadowMapTransform[FindCubeFace(litVec)]);
			//test 수정필요
			float bias = 0.03f * 512 * pointLight[pLitIndex].shadowMapInvSize; 
			smData.compareDistance = shadowCoord.z / shadowCoord.w;
			smData.shadowDepthBias = -bias / shadowCoord.w;
	
#ifdef USE_POINT_LIGHT_PCSS
			PCSSData pcssData;
			pcssData.frustumNear = pointLight[pLitIndex].frustumNear;
			pcssData.frustumFar = pointLight[pLitIndex].frustumFar;
			pcssData.penumbraScale = pointLight[pLitIndex].penumbraScale;
			pcssData.penumbraBlockerScale = pointLight[pLitIndex].penumbraBlockerScale;
			pcssData.bias = pointLight[pLitIndex].bias;
			pcssData.sampleSize = pointLight[pLitIndex].shadowMapInvSize;
		 
			shadowFactor = CalCubeSSMUsePCSS(pcssData, smData, shadowCubeMap[shadowMapIndex], shadowCoord);
#elif USE_POINT_LIGHT_PCF
			PCFData pcfData;
			pcfData.bias = pointLight[pLitIndex].bias;
			pcfData.sampleSize = pointLight[pLitIndex].shadowMapInvSize;		
...
#endif
		directLight += ComputePointLight(pLitCalData, mat, posW, normalW, tangentW, toEye) * shadowFactor;
	}
	for (int sLitIndex = spotLitSt; sLitIndex < spotLitEd; ++sLitIndex)
	{ 
		SpotLight sLitCalData;
		sLitCalData.position = spotLight[sLitIndex].position;
		sLitCalData.direction = spotLight[sLitIndex].direction;
		sLitCalData.color = spotLight[sLitIndex].color;
		sLitCalData.power = spotLight[sLitIndex].power;
		sLitCalData.frustumFar = spotLight[sLitIndex].frustumFar;
		sLitCalData.innerConeAngle = spotLight[sLitIndex].innerConeAngle;
		sLitCalData.outerConeAngle = spotLight[sLitIndex].outerConeAngle;
		...
		if (spotLight[sLitIndex].hasShadowMap)
		{
			...
			PCSSData pcssData;
			pcssData.frustumNear = spotLight[sLitIndex].frustumNear;
			pcssData.frustumFar = spotLight[sLitIndex].frustumFar;
			pcssData.penumbraScale = spotLight[sLitIndex].penumbraScale;
			pcssData.penumbraBlockerScale = spotLight[sLitIndex].penumbraBlockerScale;
			pcssData.bias = spotLight[sLitIndex].bias;
			pcssData.sampleSize = spotLight[sLitIndex].shadowMapInvSize; 
	
			shadowFactor = CalSSMUsePCSS(csmData, shadowMaps[spotLight[sLitIndex].shadowMapIndex], shadowPosH, posW);
#elif USE_SPOT_LIGHT_PCF
			PCFData pcfData;
			pcfData.bias = spotLight[sLitIndex].bias;
			pcfData.sampleSize = spotLight[sLitIndex].shadowMapInvSize;
	
			shadowFactor = CalSSMUsePCF(pcfData, shadowMaps[spotLight[sLitIndex].shadowMapIndex], shadowPosH);
#else
			shadowFactor = GetSSM(shadowMaps[spotLight[sLitIndex].shadowMapIndex], shadowPosH, spotLight[sLitIndex].bias);
#endif
		}
#endif
		directLight += ComputeSpotLight(sLitCalData, mat, posW, normalW, tangentW, toEye) * shadowFactor;
	}
	for (int rLitIndex = rectLitSt; rLitIndex < rectLitEd; ++rLitIndex)
	{ 
		if (rectLight[rLitIndex].hasShadowMap)
		{
		}
		directLight += ComputeRectLight(rectLight[rLitIndex], mat, posW, normalW, tangentW, toEye);
	}
