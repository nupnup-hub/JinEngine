...
#define DISNEY_DIFFUSE_FACTOR 0.85f
...
float3 CalBxDF(const float3 normal, float3 tangent, const float3 lightVec, const float3 viewVec, const Material mat)
{
	...
	const float3 diffColor = DisneyDiffuse(mat.albedoColor.xyz, normal, lightVec, viewVec, halfVec, mat.roughness, DISNEY_DIFFUSE_FACTOR);
...
float3 PolygonIrradiance(float3 pos, float3 points[4], uint twoSided, int sourceTextureIndex)
{
	float3 lightTexture = float3(1, 1, 1);
	if (sourceTextureIndex != -1)
	{
		lightTexture = FetchDiffuseFilteredTexture(points, textureMaps[sourceTextureIndex]);
		lightTexture = pow(lightTexture, float3(2.2f, 2.2f, 2.2f));
	}
#if 1
	float3 nPoint[5];
	nPoint[0] = points[0];
	...
	return float3(sum, sum, sum) * lightTexture;
#else 
...
float3 ComputeDirectionalLight(DirectionalLight light, Material mat, float3 normalW, float3 tangent, float3 toEye)
{
	...
	return light.color * nDotL * CalBxDF(normalW, tangent, lightVec, toEye, mat);
}
...
float3 ComputeSpotLight(SpotLight light, Material mat, float3 posW, float3 normalW, float3 tangent, float3 toEye)
{
	...
	float smoothFactor = clamp((dDotL - light.outerConeAngle) / epsilon, 0.0f, 1.0f);
...
float3 ComputeRectLight(RectLightData light, Material mat, float3 posW, float3 normalW, float3 tangent, float3 toEye)
{
	...
	float3 diffuse = PolygonIrradiance(posW, rPoints, light.isTwoSide, light.sourceTextureIndex);
	diffuse *= mat.albedoColor.xyz;
	
	[unroll]
	for (int i = 0; i < 4; ++i)
		rPoints[i] = mul(rPoints[i] - posW, ltcM);
	
	float3 irradiance = PolygonIrradiance(posW, rPoints, light.isTwoSide, light.sourceTextureIndex);
	...
	return lightColor * light.power * irradiance * (specular + diffuse);
}
