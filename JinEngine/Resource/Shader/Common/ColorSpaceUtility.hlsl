...
#define ApplySRGBCurveBody(x) x < 0.0031308 ? x * 12.92 : 1.055 * pow(x, 1.0 / 2.4) - 0.055
#define RemoveSRGBCurveBody(x) x < 0.04045 ? x / 12.92 : pow((x + 0.055) / 1.055, 2.4)

#define ApplySRGBCurve_FastBody(x) x < 0.0031308 ? 12.92 * x : 1.13005 * sqrt(x - 0.00228) - 0.13448 * x + 0.005719
#define RemoveSRGBCurve_FastBody(x) x < 0.04045 ? x / 12.92 : -7.43605 * x - 31.24297 * sqrt(-0.53792 * x + 1.279924) + 35.34864

#define ApplyREC709CurveBody(x) x < 0.0181 ? 4.5 * x : 1.0993 * pow(x, 0.45) - 0.0993
#define RemoveREC709CurveBody(x) x < 0.08145 ? x / 4.5 : pow((x + 0.0993) / 1.0993, 1.0 / 0.45)

float3 ApplySRGBCurve(float3 v)
{
	// Approximately pow(x, 1.0 / 2.2) 
	return float3(ApplySRGBCurveBody(v.x), ApplySRGBCurveBody(v.y), ApplySRGBCurveBody(v.z));
}

float3 RemoveSRGBCurve(float3 v)
{ 
	// Approximately pow(x, 2.2) 
	return float3(RemoveSRGBCurveBody(v.x), RemoveSRGBCurveBody(v.y), RemoveSRGBCurveBody(v.z));
}

// These functions avoid pow() to efficiently approximate sRGB with an error < 0.4%.
float3 ApplySRGBCurve_Fast(float3 v)
{
	return float3(ApplySRGBCurve_FastBody(v.x), ApplySRGBCurve_FastBody(v.y), ApplySRGBCurve_FastBody(v.z));
}

float3 RemoveSRGBCurve_Fast(float3 v)
{
	return float3(RemoveSRGBCurve_FastBody(v.x), RemoveSRGBCurve_FastBody(v.y), RemoveSRGBCurve_FastBody(v.z));
}
...
float3 ApplyREC709Curve(float3 v)
{ 
	return float3(ApplyREC709CurveBody(v.x), ApplyREC709CurveBody(v.y), ApplyREC709CurveBody(v.z));
}

float3 RemoveREC709Curve(float3 v)
{ 
	return float3(RemoveREC709CurveBody(v.x), RemoveREC709CurveBody(v.y), RemoveREC709CurveBody(v.z));
}
