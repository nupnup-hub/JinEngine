...
namespace JinEngine::Editor
{
	namespace Private
	{
			...
			const bool checkMouseInWindow)
		{
			...
			{
				if (!JGui::IsMouseInRect(windowPos + sceneImageMinPoint, windowSize))
					return false;
			}
			 
			return sceneImageMinPoint.Contained(windowPos, windowSize);
		}
			...
			const JVector2<float>& sceneImageMinPoint)
		{
			...
			const JVector2<float> localMousePos = mousePos - (JGui::GetWindowPos() + sceneImageMinPoint);
			...
			const float pointZValue = 1.0f)
		{
			...
			const JVector2<float> ndcPos = ToNdcPos(cam, mousePos, sceneImageMinPoint);
	...
	{
		if (!Private::IsValidData(scene, cam, sceneImageMinPoint, true))
			return nullptr;

		//계산에서 구한 pos는 world변환 후 ray dir로 사용 ray pos는 cam eye를 world로 변환한값이다
		JVector2<float> viewPos = Private::ToViewPos(cam, JGui::GetMousePos(), sceneImageMinPoint);
		...
		const JVector2<float>& maxMousePos) noexcept
	{
		...
		if (!Private::IsValidData(scene, cam, sceneImageMinPoint, isSameMosuePos))
			return std::vector<JUserPtr<JGameObject>>{};
		
		if (isSameMosuePos)
			return std::vector<JUserPtr<JGameObject>>{};

		JVector2<float> posV00 = Private::ToViewPos(cam, minMousePos, sceneImageMinPoint);
		JVector2<float> posV01 = Private::ToViewPos(cam, maxMousePos, sceneImageMinPoint);
		...
		const JVector3<float> centerPosV = (minRayV + maxRayV) * 0.5f;
		const JVector3<float> extentsPosV = (maxRayV - minRayV) * 0.5f;

		const JVector3<float> rayW00 = XMVector3Normalize(XMVector3TransformNormal(rayV00.ToXmV(), invView));
		const JVector3<float> rayW01 = XMVector3Normalize(XMVector3TransformNormal(rayV01.ToXmV(), invView));
		const JVector3<float> minRayW = JVector3<float>::Min(rayW00, rayW01) * cam->GetFarViewWidth();
		const JVector3<float> maxRayW = JVector3<float>::Max(rayW00, rayW01) * cam->GetFarViewWidth();
		const JVector3<float> posOffsetW = (minRayW + maxRayW) * 0.5f;
