...
namespace JinEngine
{
	...
	{
		std::vector<JVector3<float>>bindTarLocalT(JSkeletonFixedData::maxJointCount);
		std::vector<JVector3<float>>bindTarWorldT(JSkeletonFixedData::maxJointCount);
		...
		std::vector<JVector3<float>>& bindTargetWorldT)noexcept
	{
		...
		for (uint i = 0; i < JSkeletonFixedData::avatarBoneIndexCount; ++i)
		{
			...
			if (i != JSkeletonFixedData::rootJointIndex)
			{
				...
				bindTargetLocalT[tarIndex] = XMVectorScale(bindTarLocalT, lengthRate);
			}
			else
			{
				...
				XMStoreFloat3(&bindTarLocalTDistance, bindTarLocalTDistanceV);
				float lengthRate = bindSrcLocalTDistance.x / bindTarLocalTDistance.x;
				bindTargetLocalT[tarIndex] = XMVectorScale(bindTarT, lengthRate);
			}
		}
	}
		...
		uint8 refIndex)noexcept
	{
		...
		for (const auto& childRefIndex : data->second)
		{
			...
			const XMVECTOR childLocal = bindTargetLocalT[tarChildIndex].ToXmV();
			const XMVECTOR parentWorld = bindTargetWorldT[tarIndex].ToXmV();
			const XMVECTOR childWorld = XMVectorAdd(childLocal, parentWorld);
			bindTargetWorldT[tarChildIndex] = childWorld;
		...
		std::vector<JAnimationAdditionalBind>& additionalBind)noexcept
	{
		...
		std::vector<JMatrix4x4> bindPose(jointCount);
		for (uint i = 0; i < jointCount; ++i)
			bindPose[i].StoreXM(srcSkeleton->GetBindPose(i));
		...
		for (uint i = 1; i < JSkeletonFixedData::avatarBoneIndexCount; ++i)
		{
			...
			const XMMATRIX modM = bindPose[srcIndex].LoadXM();
			...
			additionalBind[i].transform.StoreXM(toModM);
		}
	}
		...
		std::vector<JMatrix4x4>& modBindPose)noexcept
	{
		...
		for (uint i = 0; i < JAnimationFixedData::defaultIKSlotCount; ++i)
		{
			if (updateData->ikCount[i] > 0)
			{
				for (uint j = 0; j < updateData->ikCount[i]; ++j)
				{
					...
					const XMMATRIX modM = updateData->ikJoint[i][j].movedTransform.LoadXM();
					const XMMATRIX srcM = srcSkeleton->GetBindPose(jointIndex);
					const XMMATRIX toModM = XMMatrixMultiply(modM, XMMatrixInverse(nullptr, srcM));
					additionalBind[jointRefIndex].transform.StoreXM(toModM);
					additionalBind[jointRefIndex].isIKModified = true;
					modBindPose[jointIndex].StoreXM(modM);
				}
			}
		}
		...
		for (uint i = 1; i < JSkeletonFixedData::avatarBoneIndexCount; ++i)
		{
			...
			{
				const XMMATRIX srcParentAdditionalM = additionalBind[parentRef].transform.LoadXM();
				...
				additionalBind[i].transform.StoreXM(srcAdditionalM);
				additionalBind[i].isIKModified = true;
				modBindPose[srcIndex].StoreXM(srcWorldBindMod);
			}
		}
	}
		...
		std::vector<JMatrix4x4>& modBindPose)noexcept
	{
		...
		const XMMATRIX srcWorldModBind = modBindPose[srcIndex].LoadXM();
		const XMMATRIX srcChildWorldModBind = modBindPose[srcChildIndex].LoadXM();
		...
		modBindPose[srcIndex].StoreXM(srcWorldFinalBind);
		...
		std::vector<JMatrix4x4>& modBindPose)noexcept
	{
		...
		modBindPose[srcChildIndex].StoreXM(srcChildFinalBind);
		...
		std::vector<JMatrix4x4>& modBindPose)noexcept
	{
		...
		const XMVECTOR leftShoulderParentQ = srcSkeleton->GetBindQuaternion(leftShoulderParentIndex).ToXmV();
		const XMVECTOR rightShoulderParentQ = srcSkeleton->GetBindQuaternion(rightShoulderParentIndex).ToXmV();
		...
		additionalBind[leftShoulderRef].transform.StoreXM(leftShoulderAdd);
		additionalBind[rightShoulderRef].transform.StoreXM(rightShoulderAdd);
