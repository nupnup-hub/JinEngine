...
namespace JinEngine
{
	...
	std::wstring JCUtil::GetPathWithOutFormat(const std::wstring& path)noexcept
	{
		std::wstring folderpath;
		std::wstring name;
		std::wstring format;
	...
	{
		std::string number = EraseSideChar(str, ' ');
		int dotIndex = (int)number.find_first_of(".");
		if (dotIndex == -1)
	...
	std::wstring JCUtil::ChangeWord(const std::wstring& oriSentence, const std::wstring& fromWord, const std::wstring& toWord)
	{
		std::wstring newSentence;
		size_t preIndex = 0;
		size_t index = oriSentence.find_first_of(fromWord);
		while (index != std::string::npos)
		{
			newSentence += oriSentence.substr(preIndex, index);
			newSentence += toWord;
			preIndex = index + fromWord.size();
			index = oriSentence.find_first_of(fromWord, preIndex);
		}
		newSentence += oriSentence.substr(preIndex);
		return newSentence;
	}
	std::string JCUtil::ChangeWord(const std::string& oriSentence, const std::string& fromWord, const std::string& toWord)
	{
		...
		while (index != std::string::npos)
		{  
			newSentence += oriSentence.substr(offset, index - offset); 
			newSentence += toWord;
			offset = index + fromWord.size();
			index = oriSentence.find_first_of(fromWord, offset);
		} 
	...
	{  
		return src < 'a' ? (src == tar || (src + Private::alpahbetGap) == (int)tar)
			: (src == tar || (src - Private::alpahbetGap) == (int)tar);
	}
	bool JCUtil::IsSameAlphabet(const wchar_t src, const wchar_t tar)noexcept
	{
		return src < L'a' ? (src == tar || (src + Private::alpahbetGap) == (int)tar)
			: (src == tar ||(src - Private::alpahbetGap) == (int)tar);
	}
