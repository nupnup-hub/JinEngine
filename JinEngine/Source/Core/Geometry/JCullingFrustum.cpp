...
namespace JinEngine
{
	...
	namespace Core
	{
			...
			:fNear(frustum.Near), fFar(frustum.Far), pos(frustum.Origin)
		{
			...
			XMVECTOR f = XMVector3Normalize(XMVector3Rotate(JMathHelper::VectorForward(), q));
			XMVECTOR u = XMVector3Normalize(XMVector3Rotate(JMathHelper::VectorUp(), q));
			XMVECTOR r = XMVector3Normalize(XMVector3Rotate(JMathHelper::VectorRight(), q));
		...
		J_CULLING_RESULT JCullingFrustum::IsBoundingBoxIn(_In_ const DirectX::BoundingBox& box, _Inout_ J_CULLING_FLAG& flag)const noexcept
		{
