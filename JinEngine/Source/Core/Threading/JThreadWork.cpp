...
namespace JinEngine::Core
{
	/*
	why use atomic
		- C++ 의 모든 객체들은 수정 순서(modification order) 라는 것을 정의할 수 있으며 시간에 따른 값의 변화이며 서로 다른 스레드는 현재 다른 값을 관찰할 수 있으며(최종적으로 같은 값)
	...
	*/
	...
	{ 
		...
		while (isEmpty || control.isWait.load(std::memory_order_acquire))
		{
			...
			if (control.spinCount.fetch_add(1) >= sync.maxSpinLockCount)
			{
				...
				//std::cout << std::to_string(tlsIndex)  +  " EntryKernelWait\n";
				sync.waitEvent.Wait();
				control.spinCount = 0;
				if (!control.canWork)
					break;
	...
	{
		//std::cout << std::to_string(info.uniqueID) + ": ExecuteJob\n"; 
