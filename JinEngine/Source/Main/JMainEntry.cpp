...
{
public:
	static void Run(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR pCmdLine, int nShowCm)
	{
		JinEngine::JMain{ hInstance, pCmdLine }.Run();
	}
};
...
{  
	try
	{
		JMainEntry::Run(hInstance, hPrevInstance, pCmdLine, nShowCm);
	}
	catch (const JinEngine::Core::JException& e)
	{ 		
		MessageBox(GetDesktopWindow(), e.what().c_str(), e.GetType().c_str(), MB_OK | MB_ICONEXCLAMATION);
	}
	catch (const std::exception& e)
	{		
		MessageBoxA(GetDesktopWindow(), e.what(), "Standard Exception", MB_OK | MB_ICONEXCLAMATION);
	}
	catch (...)
	{
