...
{ 
	...
	namespace
	{
		...
		static void ComputeFrustumPoint(const BoundingFrustum& camFrustum, XMVECTOR(&wCorner)[JCsmOption::maxCountOfSplit])
		{
		...
		static void ComputeNearFar(const XMVECTOR(&sceneBBoxCornerLV)[JCsmOption::maxCountOfSplit],
        ...
        const DebugDataSet& debugSet)
    {
        ...
        for (uint i = 0; i < count; ++i)
            CalculateShadowMap(lightView, sceneBBoxW, target[i].info->GetFrustum(), mapSize, i, debugSet);
    }
	...
	{   
		...
		XMFLOAT3 sceneBBoxCornerW[JCsmOption::maxCountOfSplit];
		sceneBBoxW.GetCorners(sceneBBoxCornerW);

		XMVECTOR sceneBBoxCornerLV[JCsmOption::maxCountOfSplit];
		for (uint i = 0; i < 8; ++i)
		{
		...
		for (uint i = 0; i < result.subFrustumCount; ++i)
		{
			...
			XMVECTOR wCorner[JCsmOption::maxCountOfSplit];
			ComputeFrustumPoint(subFrustum, wCorner);

			XMVECTOR lightCamOrthoMin = JVector4<float>::PositiveInfV().ToXmV();
			XMVECTOR lightCamOrthoMax = JVector4<float>::NegativeInfV().ToXmV();
			...
			for (uint j = 0; j < 8; ++j)
			{
				XMVECTOR lightConer = XMVector4Transform(wCorner[i], lightView);
				lightCamOrthoMin = XMVectorMin(lightConer, lightCamOrthoMin);
				lightCamOrthoMax = XMVectorMax(lightConer, lightCamOrthoMax);
				...
				XMVECTOR vDiagonal = wCorner[0] - wCorner[6];
				vDiagonal = XMVector3Length(vDiagonal);
 
				// The offset calculated will pad the ortho projection so that it is always the same size 
				// and big enough to cover the entire cascade interval.
				XMVECTOR vBoarderOffset = (vDiagonal - (lightCamOrthoMax - lightCamOrthoMin)) * XMVectorSet(0.5f, 0.5f, 0.5f, 0.5f);
				// Set the Z and W components to zero.
				vBoarderOffset *= multiplySetzwToZeroV;

				// Add the offsets to the projection.
				lightCamOrthoMax += vBoarderOffset;
				lightCamOrthoMin -= vBoarderOffset;
			...
			if (canMoveLightTexelSize)
			{
				...
				lightCamOrthoMin /= worldUnitsPerTexel;
				lightCamOrthoMin = XMVectorFloor(lightCamOrthoMin);
				lightCamOrthoMin *= worldUnitsPerTexel;

				lightCamOrthoMax /= worldUnitsPerTexel;
				lightCamOrthoMax = XMVectorFloor(lightCamOrthoMax);
				lightCamOrthoMax *= worldUnitsPerTexel;
			}

			float outFrustumNear = 0.0f;
			float outFrustumFar = 10000.0f;

			//5. calculate frustum near far	 
			ComputeNearFar(sceneBBoxCornerLV, lightCamOrthoMin, lightCamOrthoMax, outFrustumNear, outFrustumFar);
            outFrustumNear += debugSet.nearOffset;
            outFrustumFar += debugSet.farOffset;

            const XMMATRIX shadowProjM = XMMatrixOrthographicOffCenterLH(XMVectorGetX(lightCamOrthoMin), XMVectorGetX(lightCamOrthoMax),
                XMVectorGetY(lightCamOrthoMin), XMVectorGetY(lightCamOrthoMax),
                outFrustumNear, outFrustumFar);
