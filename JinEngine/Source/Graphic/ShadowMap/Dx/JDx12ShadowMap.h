...
namespace JinEngine
{
	...
	namespace Graphic
	{
		...
		private:
			...
			std::unique_ptr<JDx12GraphicShaderDataHolder> gShaderData[(uint)J_SHADOW_MAP_TYPE::COUNT][(uint)J_MESHGEOMETRY_TYPE::COUNT];
		public:
		...
		private:
			void BindLightFrameResource(ID3D12GraphicsCommandList* cmdList, JDx12FrameResource* dx12Frame, const JDrawHelper& helper);
		private:
			void BuildRootSignature(ID3D12Device* device);
			void BuildPso(ID3D12Device* device, const DXGI_FORMAT dsvFormat, const J_SHADOW_MAP_TYPE smType);
		};
	}
}
