...
namespace JinEngine
{
	...
	JGameObject* JGameObjectFactoryUtility::CreateShape(JGameObject& parent, const std::wstring& name, const size_t guid, const J_OBJECT_FLAG flag, const J_DEFAULT_SHAPE shape)
	{
		...
		JCFU::CreateRenderItem(Core::MakeGuid(), *newGameObj, JResourceManager::Instance().GetDefaultMeshGeometry(shape));
		return newGameObj;
	}
	...
	{
		JGameObject* newGameObj = JGFI::Create(name, Core::MakeGuid(), flag, parent);
		JCFU::CreateCamera(Core::MakeGuid(), *newGameObj, isMainCamera);
		return newGameObj;
	}
	...
	{
		JGameObject* newGameObj = JGFI::Create(name, Core::MakeGuid(), flag, parent);
		JCFU::CreateLight(Core::MakeGuid(), *newGameObj, type);
		return newGameObj;
	}
