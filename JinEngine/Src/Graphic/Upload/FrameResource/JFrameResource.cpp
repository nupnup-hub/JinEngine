...
namespace JinEngine
{
	namespace Graphic
	{
		...
		{  
			cmdListAlloc.resize(gInfo.frameThreadCount);
			commandList.resize(gInfo.frameThreadCount);
			for (uint i = 0; i < gInfo.frameThreadCount; ++i)
			{
		...
		uint JFrameResource::GetElementCount(const J_UPLOAD_FRAME_RESOURCE_TYPE type)
		{
